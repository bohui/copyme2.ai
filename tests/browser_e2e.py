from __future__ import annotations

import argparse
from pathlib import Path

from playwright.sync_api import expect, sync_playwright


def answer(page, text: str) -> None:
    page.get_by_role("textbox", name="Your story answer").fill(text)
    page.get_by_role("button", name="Save answer").click()


def main() -> None:
    parser = argparse.ArgumentParser(description="Run the anonymous-to-account Memoir browser journey.")
    parser.add_argument("--base-url", default="http://127.0.0.1:3010")
    args = parser.parse_args()

    page_errors: list[str] = []
    failed_requests: list[str] = []
    artifact_dir = Path(__file__).resolve().parents[1] / "output" / "playwright"
    artifact_dir.mkdir(parents=True, exist_ok=True)

    with sync_playwright() as playwright:
        browser = playwright.chromium.launch(headless=True)
        page = browser.new_page(viewport={"width": 1440, "height": 960}, device_scale_factor=1)
        page.on("pageerror", lambda error: page_errors.append(str(error)))
        page.on("requestfailed", lambda request: failed_requests.append(f"{request.method} {request.url}: {request.failure}"))

        page.goto(args.base_url, wait_until="networkidle")
        expect(page.get_by_role("heading", name="Your story begins with one gentle question.")).to_be_visible()
        page.get_by_role("button", name="Begin my story").click()
        expect(page.get_by_text("Round 1 of 5")).to_be_visible()

        for round_number in range(1, 6):
            answer(page, f"A remembered detail from story round {round_number}.")
            if round_number < 5:
                expect(page.get_by_text(f"Round {round_number + 1} of 5")).to_be_visible()

        expect(page.get_by_role("heading", name="Your first five memories are ready.")).to_be_visible()
        expect(page.get_by_role("button", name="Continue with Google")).to_be_visible()
        expect(page.get_by_role("button", name="Continue with Facebook")).to_be_visible()
        page.screenshot(path=str(artifact_dir / "anonymous-five-rounds.png"), full_page=True)

        page.reload(wait_until="networkidle")
        expect(page.get_by_role("heading", name="Your first five memories are ready.")).to_be_visible()

        assert not page_errors, "Browser page errors: " + "; ".join(page_errors)
        assert not failed_requests, "Failed browser requests: " + "; ".join(failed_requests)
        browser.close()


if __name__ == "__main__":
    main()
